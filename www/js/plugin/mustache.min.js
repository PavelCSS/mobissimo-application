...
{".":this.view};this.parent=b}function r(){this.cache={}}var C=/\s*/,A=/\s+/,D=/\S/,B=/\s*=/,E=/\s*\}/,F=/#|\^|\/|>|\{|&|=|!/,G=RegExp.prototype.test,H=Object.prototype.toString,x=Array.isArray||function(a){return"[object Array]"===H.call(a)},I={"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;","/":"&#x2F;"};w.prototype.eos=function(){return""===this.tail};w.prototype.scan=function(a){return(a=this.tail.match(a))&&0===a.index?(a=a[0],this.tail=this.tail.substring(a.length),this.pos+=a.length,
...
else d=[];m=q=!1}}if(!f.scan(c[0]))break;q=!0;h=f.scan(F)||"name";f.scan(C);"="===h?(s=f.scanUntil(B),f.scan(B),f.scanUntil(c[1])):"{"===h?(s=f.scanUntil(new RegExp("\\s*"+u("}"+n[1]))),f.scan(E),f.scanUntil(c[1]),h="&"):s=f.scanUntil(c[1]);if(!f.scan(c[1]))throw Error("Unclosed tag at "+f.pos);v=[h,s,p,f.pos];e.push(v);if("#"===h||"^"===h)k.push(v);else if("/"===h){h=k.pop();if(!h)throw Error('Unopened section "'+s+'" at '+p);if(h[1]!==s)throw Error('Unclosed section "'+h[1]+'" at '+p);}else"name"===
    ...
    l.clearCache=function(){return y.clearCache()};l.parse=function(a,b){return y.parse(a,b)};l.render=function(a,b,g){return y.render(a,b,g)};l.to_html=function(a,b,g,e){a=l.render(a,b,g);if(q(e))e(a);else return a};l.escape=function(a){return String(a).replace(/[&<>"'\/]/g,function(a){return I[a]})};l.Scanner=w;l.Context=m;l.Writer=r});